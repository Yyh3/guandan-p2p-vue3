...
| **v0.4.19** | ✅ 当前 | V0419 follow-up 4 项 | `network.js selectNextHostCandidate()` 确定性 UUID 字典序 + canHost 选举(v2.1 旧版重命名 `selectNextHostBySeat` 保留);`selfInfo` 加 `canHost` + `hostAddress` 字段(peer:join 上报能力);`broadcastPeerLeave` payload 加 `newHostAddress`(自动从 peers 提取);`close({broadcast:true, newHostSeat, newHostAddress})` 关闭前广播完整新 host 信息(简化 TOMBSTONE)。`requestPromoteToHost` 集成新选举 + `canHostAsNewHost()` 守卫:浏览器 ws joiner 走 host:lost 跳首页;AndroidWs native / Node ws joiner 走 rebuildAsHost 起新 server。已知未做留 v0.4.20+:mDNS / UDP 广播 / 固定服务等真正第二发现通道(scope 大需 native);41 套件 / 1985/0 单测 |
...
**当前版本**:`v0.4.19`(2026-06-30,V0419 follow-up 4 项 — `selectNextHostCandidate` 确定性 UUID 字典序 + canHost 选举 + `selfInfo` 加 `canHost` + `hostAddress` + `broadcastPeerLeave` payload 加 `newHostAddress` + `close({broadcast:true})` 关闭前广播完整新 host 信息;`requestPromoteToHost` 集成新选举 + `canHostAsNewHost()` 守卫;已知未做留 v0.4.20+:mDNS / UDP 广播 / 固定服务等真正第二发现通道;41 套件 / 1985/0 单测)
**首发目标**:v1.0.0(H5)

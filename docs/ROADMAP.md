...
| **v0.4.22** | ✅ 当前 | P1 真正的第二发现通道 + P0 安全/UX-P1/P3 收尾 + 弱网压测 | `network.js` `scanLanRooms()` 通过 HTTP `/room-info` + WS `ROOM_PROBE/ROOM_PROBE_ACK` 主动扫描局域网 host;原生环境增加 mDNS (`capacitor-zeroconf`) 作为第二发现通道;`network-transport-ws.js` 提供 `/room-info`;`JoinView.vue` 增加扫描入口与结果列表;`guandan-game.js` `_applySnapshot` 原子提交与严格校验;transport 层跟踪 `_hostSeat`;移动端横屏 UX-P1 适配;新增首页横屏/设置页双栏/全局 haptics (`@capacitor/haptics`);新增 `haptics.test.js` / `network-mdns.test.js` / `network-weaknet.test.js`;新增首页/设置/对局横屏 E2E;55 套件 / 1958/0 单测,Playwright E2E 12/0 |

...
**当前版本**:`v0.4.18`(2026-06-29,V0414-04 本地选举协议最小修复 — `requestPromoteToHost` 扩展本地 self-loop 到所有 transport + `rebuildAsHost` 失败 emit host:lost + `onHostMigrated` rebuildAsHost promise.catch 也 emit host:lost;浏览器 ws joiner 无 server 能力走 host:lost → 跳首页;AndroidWs native / Node ws joiner 走 rebuildAsHost 成功;已知未做留 v0.4.19+:确定性本地选举 / canHost + hostAddress / mDNS / UDP 第二发现通道;40 套件 / 1947/0 单测)
**首发目标**:v1.0.0(H5)

...
console.log('\n=== 1. WS host relay: seat 1 PLAY → seat 2 + seat 3 都收到 from=1 ===')
{
  ...
  // ★ 核心:让 host transport 收到来自 seat=1 joiner 的 READY 消息
  ...
  const readyMsg = { type: 'READY', from: 1, payload: { seat: 1, ready: true }, ts: Date.now() }
  host._emit(readyMsg)
  await settle(50)

  // 验证 joiner seat=2 / seat=3 收到 from=1 的 READY
  const r2 = recv2.find(m => m.type === 'READY')
  const r3 = recv3.find(m => m.type === 'READY')
  assert('joiner seat=2 收到 READY (relay 命中)', r2 != null)
  assert('joiner seat=3 收到 READY (relay 命中)', r3 != null)
  ...
  eq('payload.ready=true', r2?.payload?.ready, true)

  // 验证:原 sender seat=1 不应收到自己 READY 的回环
  assert('原 sender seat=1 不收到自己 READY 的回环', !recv1.some(m => m.type === 'READY'))
...
//   (READY 自 BUG-F 修复起加入白名单,所以不测 READY)
// ============================================================
console.log('\n=== 2. relay 白名单:HEARTBEAT / _DISCONNECT 不被转发 ===')
{
  ...
  // 静态审查 BUG-F 修复:READY 已加入白名单(从 joiner 转发到其它 joiner),
  //   测非白名单消息时移除 READY。
  const testMsgs = [
  ...
  // 这些消息 _handleHostMessage 内部对 HEARTBEAT/_DISCONNECT 是状态更新,可能产生副作用
...
console.log('\n=== 3. BC host 模式不调 relayFromClient (BC 天然广播,避免双倍消息) ===')
{
  ...
  // BC host 收到 READY 消息 — relayFromClient 应跳过
  bcSendCalls = []
  const readyMsg = { type: 'READY', from: 1, payload: { seat: 1, ready: true } }
  bcTransport._emit(readyMsg)
  await settle(30)

  // BC 模式 _handleHostMessage 对 READY 不处理分支,无副作用
  // relayFromClient 也跳过 (因为 _channel != null)
  const relayReady = bcSendCalls.filter(m => m.type === 'READY')
  eq('BC host 模式不触发 relayFromClient (to 定向 send=0)', relayReady.length, 0)
...
console.log('\n=== 4. joiner 端收到消息不调 relayFromClient(joiner 不是 host) ===')
{
  ...
  // 模拟 joiner 收到 host 发来的 READY 消息 (from=0 host 发的)
  joinerSendCalls = []
  const readyMsg = { type: 'READY', from: 0, payload: { seat: 0, ready: true }, to: 1 }
  joinerTransport._emit(readyMsg)
  await settle(30)

  // joiner 端不调 relayFromClient (因为 isHostFlag=false),所以 send 不应增加
  eq('joiner 收到 READY 不产生 send 调用 (relay 仅 host 触发)', joinerSendCalls.length, 0)
...
console.log('\n=== 5. relay 目标:跳过原 sender + host 自己 ===')
{
  ...
  host._emit({ type: 'READY', from: 2, payload: { seat: 2, ready: true } })
  await settle(30)

  const r1 = recv1.find(m => m.type === 'READY')
  const r3 = recv3.find(m => m.type === 'READY')
  assert('READY from=2 → seat=1 收到', r1 != null)
  assert('READY from=2 → seat=3 收到', r3 != null)
  eq('relay to seat=1 (定向)', r1?.to, 1)
  eq('relay to seat=3 (定向)', r3?.to, 3)
  eq('relay from=2 (原 sender 保留)', r1?.from, 2)

  // 原 sender seat=2 不应收到自己 READY 的回环
  const recv2 = []
  clients[1].on('message', (d) => { try { recv2.push(JSON.parse(d.toString())) } catch (e) {} })
  await settle(20)
  assert('原 sender seat=2 不收到自己 READY 的回环', !recv2.some(m => m.type === 'READY'))
...
console.log('\n=== 6. host peers Map 为空时 relay 安全 no-op ===')
{
  ...
  let threw = false
  try {
    hostTransport._emit({ type: 'READY', from: 1, payload: { seat: 1, ready: true } })
  } catch (e) { threw = true }
  await settle(10)
  ...
  threw = false
  try {
    hostTransport._emit({ type: 'READY', from: 0, payload: { seat: 0, ready: true } })
  } catch (e) { threw = true }
  await settle(10)

...
console.log('\n=== 23. v2.1 端到端:joiner 掉线后推进 6.5s → host 释放 (mock Date.now 验证 6-8s 窗口) ===')
{
  ...
  const _realDateNow = Date.now
  try {
    // ★ v0.4.24 行为更新:_DISCONNECT 快路径修复(座位从 payload.seat 读取)后,
    //   joiner 真正的 ws 断开会立即把 lastHeartbeat 标为 -1,host 在下一次
    //   checker tick(≤2s)即释放座位,不再等满 6s 心跳超时。
    //   旧断言("5.5s 不释放")锁定的是快路径失效的坏行为,此处反转。
    // 阶段 1:推进 5500ms → 快路径已释放
    Date.now = () => joinTs + 5500
    Host._tickHeartbeatChecker()
    await settle(50)
    assert('掉线 5.5s → host 已释放 seat=1(_DISCONNECT 快路径生效)', !Host.getPeers().has(1))
    assert('掉线 5.5s → peer:leave 已触发 seat=1', leaveEvent?.seat === 1)
    assert('掉线 5.5s → ai:takeover 已触发 seat=1', aiTakeoverEvent?.seat === 1)

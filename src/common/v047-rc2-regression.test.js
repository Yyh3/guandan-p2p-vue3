...
console.log('\n=== 5.1 host 迁移端到端状态测试(4 端 + game) ===')
{
  ...
  // 断言(P0-04 修复后):
  // - 新 host:net.isHost() === true / selfSeat === 2(座位稳定)
  // - game.state.hands[2] 等于迁移前 seat 2 的手牌(保留在原 seat)
  // - game.state.hands[0] 保留(旧 host 座位由 AI 接管,不清空)
  // - currentPlayer/lastPlay.who/trickHistory 不再重定向,游戏座位保持不变
  // - 新 host 可以继续出牌
  ...
  // P0-04 修复后:新 host 保持原 seat 2,旧 host seat 0 由 AI 接管继续参与
  // 1) hands[2] 仍等于迁移前 seat 2 的手牌
  eq('迁移后 hands[2] 等于迁移前 seat 2 的手牌(座位稳定)', st1.hands[2], seat2HandBefore)
  // 2) hands[0] 保留原牌,不清空
  eq('迁移后 hands[0] 保留原牌(旧 host 由 AI 接管)', st1.hands[0].length, seat2HandLen)
  // 3) 旧 host(seat 0) 不在 abandonedSeats,不在 finishedOrder,已加入 aiPlayers
  assert('★ 迁移后旧 host seat 0 不在 abandonedSeats', !st1.abandonedSeats?.includes(0))
  assert('★ 旧 host seat 0 不在 finishedOrder(可继续出牌)', !st1.finishedOrder.includes(0))
  assert('★ 迁移后旧 host seat 0 在 aiPlayers 中(AI 接管)', game.getAIPlayers().includes(0))
  // 4) currentPlayer 保持原座位(不应被重定向到新 host)
  // 5) 新 host 继续出牌验证
  const st2Check = game.getState()
  assert('★ 迁移后 hands[2].length=27(新 host 原 seat 手牌数)', st2Check.hands[2].length === seat2HandLen)
  assert('★ 迁移后旧 host seat 0 不在 abandonedSeats', !st2Check.abandonedSeats?.includes(0))
  ...
  // ★ 关键断言:新 host 可以继续出牌
  game._state.currentPlayer = 2  // 强制让新 host 行动
  const firstCard = st2Check.hands[2][0]
  const playRes = game.playerPlay(2, [firstCard])
  assert('★ ★ 新 host(seat 2)migrateHost 后能 playerPlay', playRes && playRes.ok === true)
...
console.log('\n=== 5.1.2 trickHistory 迁移后 _originalSeat 保留 ===')
{
  ...
  const st1 = game.getState()
  // P0-04 修复后:trickHistory 不再重映射,保留原 seat 记录
  assert('迁移后 trickHistory[0].seat=0(游戏座位不变)', st1.trickHistory[0].seat === 0)
  assert('迁移后 trickHistory[0] 无 _originalSeat', !('_originalSeat' in st1.trickHistory[0]))
}
...
console.log('\n=== 5.1.3 旧 host seat 加入 finishedOrder ===')
{
  ...
  if (newHostSeat != null) {
    ...
    // P0-04 修复后:已出完的 seat 0 保持 finishedOrder 状态,不进 abandonedSeats
    const st1 = game.getState()
    assert('★ 迁移后旧 host seat 0 不在 abandonedSeats',
...
console.log('\n=== 5.5 BUG-RC3-002 修复:applySnapshot→migrateHost 顺序不覆盖 hand remap ===')
{
  ...
  // ===== 错误顺序(模拟 BUG-RC3-002 旧行为,seat-stable 下同样会覆盖)
  const gameBad = createGame({ players: [{}, {}, {}, {}], seed: 9 })
  gameBad.deal()
  gameBad.migrateHost(0, 2)
  gameBad.applySnapshot(snapshotBefore)  // ← 错的:用旧 snapshot 覆盖 hands[0]
  const stBad = gameBad.getState()
  // P0-04 修复后:hands[0] 不会被清空,旧 snapshot 覆盖后 hands[0] 仍是旧 host 手牌
  assert('旧顺序 snapshot 覆盖后 hands[0] 是旧 host 手牌',
    JSON.stringify(stBad.hands[0]) === JSON.stringify(oldHostHand))
  assert('旧顺序 snapshot 覆盖后 hands[2] 仍是新 host 原手牌',
    JSON.stringify(stBad.hands[2]) === JSON.stringify(newHostHand))

  // ===== 正确顺序(修复后,seat-stable)=====
  ...
  // P0-04 修复后:hands[2] 保留新 host 原手牌,hands[0] 保留旧 host 手牌(AI 接管)
  assert('★ 正确顺序后 hands[2] = 新 host 的手牌',
    JSON.stringify(stGood.hands[2]) === JSON.stringify(newHostHand))
  assert('★ 正确顺序后 hands[0] 保留旧 host 手牌(AI 接管)',
    JSON.stringify(stGood.hands[0]) === JSON.stringify(oldHostHand))
  assert('★ 正确顺序后 hands[2] 不是旧 host 手牌(没被覆盖)',
    JSON.stringify(stGood.hands[2]) !== JSON.stringify(oldHostHand))
  assert('★ 正确顺序后 abandonedSeats 不含旧 host seat 0',
    !stGood.abandonedSeats.includes(0))
  assert('★ 正确顺序后 finishedOrder 不含 seat 0',
    !stGood.finishedOrder.includes(0))
  assert('★ 正确顺序后旧 host seat 0 在 aiPlayers 中',
    gameGood.getAIPlayers().includes(0))
  // currentPlayer 不再重定向,保持原座位
  if (stBefore.currentPlayer === 0 || stBefore.currentPlayer === 2) {
    assert('★ 正确顺序后 currentPlayer 保持原座位(0 或 2)',
...
console.log('\n=== 5.6 BUG-RC3-003 修复:迁移后新 host(seat 0)能继续出牌 ===')
{
  ...
  game.migrateHost(0, 2)
  const st = game._state
  // P0-04 修复后:seat 2 是新 host,他有手牌;currentPlayer 保持 0(旧 host 座位由 AI 接管)
  assert('迁移后 currentPlayer === 0(旧 host 原 seat 由 AI 接管)', st.currentPlayer === 0)
  assert('迁移后 hands[2].length > 0(新 host 有手牌)', st.hands[2].length > 0)
  assert('迁移后 hands[0].length > 0(旧 host 手牌保留,AI 接管)', st.hands[0].length > 0)
  assert('迁移后 finishedOrder 不含 2(没把新 host 标 finished)',
    !st.finishedOrder.includes(2))
  // ★ 关键:playerPlay(0, 一张牌) 应当成功(旧 host 座位由 AI 接管,仍可出牌)
  const firstCard = st.hands[0][0]
  const result = game.playerPlay(0, [firstCard])
  assert('★ 迁移后旧 host 座位(AI 接管)playerPlay(0, [card]) 返回 ok:true',
    result && result.ok === true)
  assert('★ 迁移后旧 host 座位(AI 接管)出牌成功:currentPlayer 推进到下一位(1)',
    game._state.currentPlayer === 1)
  // P0-04 修复后:旧 host seat 0 由 AI 接管,不在 abandonedSeats 中
  assert('★ 旧 host seat 0 不在 abandonedSeats',
    !game.getState().abandonedSeats.includes(0))
  assert('★ 旧 host seat 0 在 aiPlayers 中(AI 接管)',
    game.getAIPlayers().includes(0))
  assert('★ 旧 host 0 不在 finishedOrder(可继续出牌)',
...
console.log('\n=== 5.7 BUG-RC3-001 验证:network.requestPromoteToHost 存在 ===')
{
  const { requestPromoteToHost, selectNextHostCandidate, selectNextHostBySeat } = await import('./network.js')
  assert('★ network.requestPromoteToHost 是 function',
    typeof requestPromoteToHost === 'function')
  // v0.4.19:selectNextHostCandidate 改 UUID-based 算法 — 全空时返回 null(无候选)
  assert('★ network.selectNextHostCandidate 是 function',
    typeof selectNextHostCandidate === 'function')
  // v2.1 旧版 selectNextHostBySeat 保留作向后兼容
  assert('★ network.selectNextHostBySeat 是 function (v2.1 旧版)',
    typeof selectNextHostBySeat === 'function')
  // 新版 selectNextHostCandidate:peers 空时返回 null
  let r = selectNextHostCandidate()
  assert('★ selectNextHostCandidate() 无参调用不抛错', true)
  assert('★ selectNextHostCandidate() peers 空时返回 null(无候选)',
    r === null)
  // 旧版 selectNextHostBySeat:peers 空时返回 0
  let r2 = selectNextHostBySeat()
  assert('★ selectNextHostBySeat() 返回值在 0..3 之间',
    Number.isInteger(r2) && r2 >= 0 && r2 <= 3)
  assert('★ selectNextHostBySeat() 默认 peers 空时返回 0(无候选)',
    r2 === 0)
}

...
async function main() {
  ...
  console.log('\n=== 5.5 玩家出含鬼牌的牌 ===')
  {
    ...
    game.getState().hands[0] = [ghost, spade8, { suit: 0, rank: 17 }, { suit: 1, rank: 17 }]
    game.getState().firstPlayer = 0
    game.getState().currentPlayer = 0
    game.getState().leaderPlayer = 0
    ...
    game.getState().hands[1] = [ghost2, heart9, { suit: 0, rank: 16 }, { suit: 1, rank: 16 }]
    ...
    game.getState().currentPlayer = 2
    const ghost3 = { suit: 1, rank: 15 }
    const club10 = { suit: 2, rank: 10 }
    game.getState().hands[2] = [ghost3, club10, { suit: 0, rank: 14 }, { suit: 1, rank: 14 }]
...
console.log('\n=== 19.5 v2.1 P3:migrateHost 后旧座位加入 abandonedSeats,游戏可继续 ===')
  {
    ...
    assert('nextTurn 跳过 abandoned seat 2', game.getState().currentPlayer === 3)
  }
...
console.log('\n=== 20. v2.1 P3:网络层 selectNextHostBySeat (BC 集成,seat 优先级) ===')
{
  ...
  console.log('\n=== 32.5 行为覆盖:applyPass / nextRound / applyRoundEnd 关键状态 ===')
  {
    // applyPass 跳过被放弃座位
    {
      ...
      g.applyPass(0)
      assert('applyPass 后跳过 abandoned seat 1', g.getState().currentPlayer === 2)
    }

...
function setupGameAsFirstPlayer(target = 0, opts = {}) {
  for (let i = 0; i < 50; i++) {
    ...
    g.deal()
    if (g._state.firstPlayer === target) return g
  }
  return null
}
...
async function main() {
  ...
  console.log('\n=== 4. v3.8 P1:applyPlay/applyPass 联机同步接口 ===')
  {
    ...
    if (game) {
      ...
      assert('applyPlay 后轮到 seat 2', game._state.currentPlayer === 2)
    } else {
  ...
  console.log('\n=== 5. applyPass 联机同步 ===')
  {
    ...
    if (game) {
      ...
      // 轮到 seat 1
      game.applyPass(1)
      assert('applyPass 后轮到 seat 2', game._state.currentPlayer === 2)
      game.applyPass(2)
      game.applyPass(3)
      // 3 次 pass 后,新一轮,currentPlayer 回到 leader (seat 0)
      assert('3 次 pass 后新一轮回到 seat 0', game._state.currentPlayer === 0)
      assert('新一轮 lastPlay 清空', game._state.lastPlay === null)
    } else {
  ...
  console.log('\n=== 5.5 玩家出含鬼牌的牌 ===')
  {
    // 构造一个确定手牌:seat 0 有红桃 2(鬼牌) + 黑桃 8,levelRank=15(打 2)
    const game = createGame({ seats: 4, levelRank: 15, aiPlayers: [], seed: 12345 })
    game.deal()
    // 强制给 seat 0 替换手牌为测试牌
    const ghost = { suit: 1, rank: 15 }
    const spade8 = { suit: 0, rank: 8 }
    game._state.hands[0] = [ghost, spade8, { suit: 0, rank: 17 }, { suit: 1, rank: 17 }]
    game._state.firstPlayer = 0
    game._state.currentPlayer = 0
    game._state.leaderPlayer = 0

    // 用鬼牌 + 8 出对 8(首家)
    const r1 = game.playerPlay(0, [ghost, spade8])
    assert('playerPlay 接受鬼牌+8 出对 8', r1.ok)
    assert('出对 8 后 lastPlay 是 PAIR', game.getState().lastPlay.type === E.TYPE.PAIR)
    assert('出对 8 后 lastPlay mainRank=8', game.getState().lastPlay.mainRank === 8)
    assert('鬼牌从手牌移除后剩 2 张', game.getState().hands[0].length === 2)

    // 轮到 seat 1,给它一张 9 让它用鬼牌跟对 9(压对 8)
    game._state.currentPlayer = 1
    const ghost2 = { suit: 1, rank: 15 }
    const heart9 = { suit: 1, rank: 9 }
    game._state.hands[1] = [ghost2, heart9, { suit: 0, rank: 16 }, { suit: 1, rank: 16 }]
    const r2 = game.playerPlay(1, [ghost2, heart9])
    assert('playerPlay 接受鬼牌+9 压对 8', r2.ok)
    assert('跟牌后 lastPlay mainRank=9', game.getState().lastPlay.mainRank === 9)

    // applyPlay 同步路径:seat 2 收到原始 cards(含鬼牌)也能应用
    game._state.currentPlayer = 2
    const ghost3 = { suit: 1, rank: 15 }
    const club10 = { suit: 2, rank: 10 }
    game._state.hands[2] = [ghost3, club10, { suit: 0, rank: 14 }, { suit: 1, rank: 14 }]
    game.applyPlay(2, [ghost3, club10])
    assert('applyPlay 同步含鬼牌出牌成功', game.getState().lastPlay.mainRank === 10)
  }
...
console.log('\n=== 6. seed 化发牌(确定性手牌) ===')
  ...
  console.log('\n=== 6b. 同步切牌:deal 支持指定首家 ===')
  {
    const g = createGame({ seats: 4, levelRank: 5, aiPlayers: [], seed: 12345 })
    g.deal(12345, 2)
    assert('deal(seed, 2) 后 firstPlayer = 2', g.getState().firstPlayer === 2)
    assert('deal(seed, 2) 后 currentPlayer = 2', g.getState().currentPlayer === 2)
    assert('deal(seed, 2) 后 leaderPlayer = 2', g.getState().leaderPlayer === 2)
    // 非法 firstSeat 应忽略,回退到 seeded 随机(仍为 0..3)
    const g2 = createGame({ seats: 4, levelRank: 5, aiPlayers: [], seed: 12345 })
    g2.deal(12345, 99)
    assert('deal(seed, 99) 非法值忽略,firstPlayer 在 0..3 内', g2.getState().firstPlayer >= 0 && g2.getState().firstPlayer <= 3)
    // 不传 firstSeat 保持原有行为
    const g3 = createGame({ seats: 4, levelRank: 5, aiPlayers: [], seed: 12345 })
    g3.deal(12345)
    assert('deal(seed) 不传 firstSeat 仍在 0..3 内', g3.getState().firstPlayer >= 0 && g3.getState().firstPlayer <= 3)
  }
...
console.log('\n=== 7. P2P 模式 aiPlayers=[] 不调 AI ===')
  ...
  console.log('\n=== 8. v3.8 P1:applyRoundEnd 联机结算 ===')
  {
    ...
    if (game) {
      ...
      game.applyRoundEnd()
      assert('applyRoundEnd 后 phase=finished', game._state.phase === 'finished')
      assert('applyRoundEnd 后 finishedOrder 是数组', Array.isArray(game.getState().finishedOrder))
      assert('applyRoundEnd 后 levelRank 是数字', typeof game._state.levelRank === 'number')
      // 再调一次不报错(幂等)
      game.applyRoundEnd()
      assert('applyRoundEnd 幂等', game._state.phase === 'finished')
    } else {
...
console.log('\n=== 10.5 v3.x P0-4:applyPass 按活跃玩家数判断 ===')
{
  ...
  if (game) {
    ...
    const hand0 = game.getState().hands[0]
    game.playerPlay(0, [hand0[0]])
    assert('出牌后轮到 seat 1', game._state.currentPlayer === 1)

    // 直接 mutate state,把 seat 2 标记为 finished(模拟残局)
    game._state.finishedOrder.push(2)
    ...
    assert('1/2 pass 后轮到 seat 3', game._state.currentPlayer === 3)
    game.applyPass(3)  // passCount=2, 应结束(2 = activePlayers - 1)
    assert('2/2 pass 后 trick 结束', game._state.lastPlay === null)
    assert('2/2 pass 后 currentPlayer = leader 0', game._state.currentPlayer === 0)
  } else {
...
console.log('\n=== 10.6 v3.x P0-4:残局 2 人活跃只需 1 pass ===')
{
  ...
  if (game) {
    ...
    game._state.finishedOrder.push(2, 3)
    // activePlayers = 2,需 1 pass
    game.applyPass(1)  // passCount=1, 应结束
    assert('残局 1 pass 即结束 trick', game._state.lastPlay === null)
  } else {
...
console.log('\n=== 10.7 v3.x P0-5:3 人出完即结束本局 ===')
{
  ...
  if (game) {
    ...
    assert('phase = finished', game._state.phase === 'finished')
  } else {
...
console.log('\n=== 10.8 v3.x P0-6:nextTurn 4 人全 finished 不死循环 ===')
{
  ...
  if (game) {
    ...
    assert('最终 phase = finished', game._state.phase === 'finished')
  } else {
...
console.log('\n=== 10.10 v3.x P2-26:_applySnapshot 畸形数据防御 ===')
{
  ...
  console.log('\n=== 11. v2.1 P3:migrateHost 座位稳定(seat 2 升级) ===')
  {
    const game = createGame({ seats: 4, levelRank: 5, aiPlayers: [] })
    game.deal()
    ...
    // 1) 新 host 手牌保留在原 seat 2,不搬到 hands[0]
    assert('新 host hands[2] 不变(仍等于旧 hands[2])',
      JSON.stringify(st1.hands[2]) === JSON.stringify(newHostHand))
    assert('新 host hands[2].length === 27', st1.hands[2].length === 27)
    // 2) 旧 host (seat 0) 手牌保留,由 AI 接管(不再清空)
    assert('旧 host hands[0] 仍为 27 张', st1.hands[0].length === 27)
    assert('旧 host hands[0] 保持原牌',
      JSON.stringify(st1.hands[0]) === JSON.stringify(oldHand0))
    // 3) 旧 host (seat 0) 不在 abandonedSeats,不在 finishedOrder
    assert('旧 host (seat 0) 不在 abandonedSeats 中', !st1.abandonedSeats.includes(0))
    assert('旧 host (seat 0) 不在 finishedOrder 里(未出完牌)', !st1.finishedOrder.includes(0))
    // 4) 旧 host (seat 0) 已加入 aiPlayers,新 host 原 seat 2 不在 aiPlayers
    assert('旧 host (seat 0) 在 aiPlayers 中(AI 接管)', game.getAIPlayers().includes(0))
    assert('新 host 原 seat 2 不在 abandonedSeats 中', !st1.abandonedSeats.includes(2))
    // 5) levelRank 保持
  ...
  console.log('\n=== 12. v2.1 P3:migrateHost 参数边界 ===')
  {
    const game = createGame({ seats: 4, levelRank: 5, aiPlayers: [] })
    game.deal()
    // Phase 1 seat-stable:任意不同合法 seat 都可迁移
    assert('migrateHost(1, 2) 返回 true', game.migrateHost(1, 2) === true)
    // 非法:newHostSeat 越界
  ...
  console.log('\n=== 13. v2.1 P3:migrateHost 调整 currentPlayer (旧 host 回合时) ===')
  {
    ...
    assert('强制后 currentPlayer=0', game._state.currentPlayer === 0)
    game.migrateHost(0, 2)
    // 迁移后 currentPlayer 保持 0:旧 host 座位保留,由 AI 接管,不是把回合交给新 host
    assert('迁移后 currentPlayer 保持 0 (旧 host 座位由 AI 接管)', game._state.currentPlayer === 0)
    assert('迁移后旧 host seat 0 在 aiPlayers 中', game.getAIPlayers().includes(0))
  }
  ...
  console.log('\n=== 14. v2.1 P3:migrateHost 调整 currentPlayer (新 host 原 seat 回合时) ===')
  {
    ...
    game.migrateHost(0, 2)
    // 迁移后 currentPlayer 保持 2(新 host 自己的原 seat)
    assert('迁移后 currentPlayer 保持 2 (新 host 原 seat)', game._state.currentPlayer === 2)
  }
  ...
  console.log('\n=== 15. v2.1 P3:migrateHost 不影响其他 joiner 的 currentPlayer ===')
  {
    ...
    assert('旁观 joiner 回合不变 (currentPlayer=1)', game._state.currentPlayer === 1)
  }
  ...
  console.log('\n=== 16. v2.1 P3:migrateHost 修正 lastPlay.who (旧 host 出的牌) ===')
  {
    ...
    game.migrateHost(0, 2)
    // lastPlay.who 保持 0:游戏座位不变,旧 host 只是由 AI 接管
    assert('lastPlay.who 保持 0 (游戏座位不变)', game.getState().lastPlay?.who === 0)
  }
  ...
  console.log('\n=== 17. v2.1 P3:migrateHost 修正 lastPlay.who (新 host 原 seat 出的牌) ===')
  {
    ...
    game.migrateHost(0, 2)
    // lastPlay.who 保持 2(新 host 原 seat 不变)
    assert('lastPlay.who 保持 2 (新 host 原 seat)', game.getState().lastPlay?.who === 2)
  }
  ...
  console.log('\n=== 18. v2.1 P3:migrateHost trickHistory seat 重映射 ===')
  {
    ...
    // 游戏座位/历史出牌记录不再重映射
    assert('trickHistory[0].seat 保持 0', h[0]?.seat === 0)
    assert('trickHistory[0] 无 _originalSeat', !('_originalSeat' in (h[0] || {})))
    assert('trickHistory[1].seat 保持 2', h[1]?.seat === 2)
    assert('trickHistory[2].seat 保持 1', h[2]?.seat === 1)
  }

  console.log('\n=== 19. v2.1 P3:migrateHost AI 接管旧 host 座位 ===')
  {
    ...
    game.migrateHost(0, 2)
    // 旧 host (0) 由 AI 继续接管
    assert('迁移后 aiPlayers 仍含 0', game.getAIPlayers().includes(0))
  }

  console.log('\n=== 19.1 v2.1 P3:migrateHost 新 host 原 seat 若为 AI 则转 human ===')
  {
    const game = createGame({ seats: 4, levelRank: 5, aiPlayers: [2] })
    game.deal()
    assert('初始 aiPlayers 含 2', game.getAIPlayers().includes(2))
    game.migrateHost(0, 2)
    // 新 host (2) 控制器是人类,不应继续是 AI
    assert('迁移后 aiPlayers 不含 2', !game.getAIPlayers().includes(2))
    assert('迁移后 aiPlayers 含 0 (旧 host 由 AI 接管)', game.getAIPlayers().includes(0))
  }

console.log('\n=== 19.5 v2.1 P3:migrateHost 后旧座位由 AI 接管,游戏可继续 ===')
  {
    const game = createGame({ seats: 4, levelRank: 5, aiPlayers: [] })
    game.deal()
    // 让 seat 0 出牌,使 currentPlayer 推进到 1,再迁移到 seat 2
    const hand0 = game.getState().hands[0]
    game.playerPlay(0, [hand0[0]])
    // currentPlayer 现在是 1
    game.migrateHost(0, 2)
    const st = game._state
    assert('migrateHost 后旧 host seat 0 不在 abandonedSeats', !st.abandonedSeats.includes(0))
    assert('migrateHost 后旧 host seat 0 由 AI 接管', game.getAIPlayers().includes(0))
    assert('migrateHost 后新 host seat 2 不在 abandonedSeats', !st.abandonedSeats.includes(2))
    // 模拟后续出牌:currentPlayer 是 1,让 1 出,然后正常轮到 seat 2
    game._applySnapshot({ currentPlayer: 1 })
    const hand1 = st.hands[1]
    game.playerPlay(1, [hand1[0]])
    // 接下来应轮到 seat 2(正常顺序,seat 0 尚未轮到)
    assert('nextTurn 轮到 seat 2', game._state.currentPlayer === 2)
  }

  console.log('\n=== 19.6 v2.1 P3:migrateHost 后整局仍能得到 4 人排名 ===')
  {
    const game = createGame({ seats: 4, levelRank: 5, aiPlayers: [] })
    game.deal()
    // 模拟已出完 3 人,只剩 seat 3 没出完
    game._applySnapshot({
      finishedOrder: [0, 1, 2],
      currentPlayer: 3,
      phase: 'playing',
      hands: [[], [], [], game.getState().hands[3].slice()],
    })
    game.migrateHost(0, 2)
    // seat 0 已出完,不应再被加入 aiPlayers
    assert('已出完的 seat 0 不在 aiPlayers', !game.getAIPlayers().includes(0))
    // 直接触发结算:applyRoundEnd 会在 finishedOrder.length===3 时把 seat 3 补为末位
    game.applyRoundEnd()
    const st = game.getState()
    assert('finishedOrder 包含 4 人', st.finishedOrder.length === 4)
    assert('finishedOrder 包含旧 host seat 0', st.finishedOrder.includes(0))
    assert('finishedOrder 末位是 seat 3', st.finishedOrder[3] === 3)
    assert('abandonedSeats 为空', st.abandonedSeats.length === 0)
  }
...
console.log('\n=== 20. v2.1 P3:网络层 selectNextHostBySeat (BC 集成,seat 优先级) ===')
{
  ...
  console.log('\n=== 22. v2.1 P3:joiner 端 PEER_LEAVE 触发 host:migrated 事件 (走 BC transport 集成) ===')
  {
    ...
    if (typeof BroadcastChannel === 'undefined') {
      ...
      // J2 端:被选为新 host,isMyself=true,但 logical seat 保持 2(座位稳定)
      ...
      assert('J2 端 selfSeat 保持 2 (座位稳定)', J2.getSelfSeat() === 2)
      assert('J2 端 hostSeat 更新为 2', J2.getHostSeat() === 2)
  ...
  console.log('\n=== 31. v0.4.9 isRestartAfterA 判定 ===')
  {
    ...
    // P1-01 修复后:只有胜方队伍升级,levelRank 序列改为 2→3→...→A
    // 场景 1:打 A(14)+ 头游我方(0)→ 双上 3 级 → 过 A(胜方已在 A,升级封顶)
    {
      const g = createGame({ players: [{}, {}, {}, {}], seed: 1 })
      g._state.levelRank = 14  // A
      g._state.teamLevels = [14, 14]
      g._state.finishedOrder = [0, 2, 1, 3]  // 队 0 头游 + 二游
      g.applyRoundEnd()
      const st = g._state
      assert('★ 打 A + 头游我方 → isRestartAfterA=true', st.isRestartAfterA === true)
      assert('★ 打 A + 头游我方 → levelUp=3(双上)', st.levelUp === 3)
      assert('★ 打 A 后 levelRank 仍封顶在 A(14)', st.levelRank === 14)
      assert('★ 打 A 后只有胜方 teamLevels[0] 升到 14(已是 A)', st.teamLevels[0] === 14)
      assert('★ 打 A 后败方 teamLevels[1] 保持 14 不变', st.teamLevels[1] === 14)
    }
    // 场景 2:打 A(14) + 头游对方 → 对方队升级,过 A
    {
      const g = createGame({ players: [{}, {}, {}, {}], seed: 2 })
      g._state.levelRank = 14
      g._state.teamLevels = [14, 14]
      // 头游对方(队 1) → 队 1 升 3 级(双上)
      g._state.finishedOrder = [1, 3, 0, 2]  // 队 1 头游 + 二游
      g.applyRoundEnd()
      const st = g._state
      assert('★ 打 A + 头游对方 → isRestartAfterA=true', st.isRestartAfterA === true)
      assert('★ 打 A + 头游对方 → 队 0 等级不变', st.teamLevels[0] === 14)
      assert('★ 打 A + 头游对方 → 队 1 等级封顶 A', st.teamLevels[1] === 14)
    }
    // 场景 3:打 2(15)→ 双上 3 级 → 不是过 A,升到 4(新序列)
    {
      const g = createGame({ players: [{}, {}, {}, {}], seed: 3 })
      g._state.levelRank = 15  // 2
      g._state.teamLevels = [15, 15]
      g._state.finishedOrder = [0, 2, 1, 3]  // 头游我方,双上 +3
      g.applyRoundEnd()
      const st = g._state
      assert('★ 打 2 + 头游我方 → isRestartAfterA=false(只是升级,没过 A)', st.isRestartAfterA === false)
      assert('★ 打 2 + 双上 3 级 → levelRank 升到 5', st.levelRank === 5)
      assert('★ 打 2 + 双上 3 级 → 只有胜方 teamLevels[0] 升到 5', st.teamLevels[0] === 5)
      assert('★ 打 2 + 双上 3 级 → 败方 teamLevels[1] 保持 15', st.teamLevels[1] === 15)
    }
  ...
  console.log('\n=== 32. v0.4.9 restartMatch 重开一局 ===')
  {
    ...
    g2._state.levelRank = 10
    g2.restartMatch()
    assert('★ restartMatch() 无参默认 levelRank=15', g2._state.levelRank === 15)
  }

  console.log('\n=== 32.5 行为覆盖:applyPass / nextRound / applyRoundEnd 关键状态 ===')
  {
    // applyPass 跳过被放弃座位
    // P1-02 修复:applyPass 必须当前玩家且有 lastPlay;用 seat 2(非领出者)测试跳过 seat 1
    {
      const g = createGame({ players: [{}, {}, {}, {}], seed: 6 })
      g.deal()
      g._applySnapshot({
        currentPlayer: 2,
        abandonedSeats: [1],
        lastPlay: { who: 0, type: E.TYPE.SINGLE, mainRank: 3, length: 1, cards: [{ suit: 0, rank: 3 }] },
      })
      g.applyPass(2)
      assert('applyPass 后跳过 abandoned seat 1', g._state.currentPlayer === 3)
    }
    // applyRoundEnd 携带 previousLevelRank
    {
      const g = createGame({ players: [{}, {}, {}, {}], seed: 7 })
      g._state.levelRank = 14
      g._state.teamLevels = [14, 14]
      g._state.finishedOrder = [0, 2, 1, 3]
      g.applyRoundEnd()
      const st = g._state
      assert('applyRoundEnd 保留 previousLevelRank=14', st.previousLevelRank === 14)
      assert('applyRoundEnd 设置 isRestartAfterA', st.isRestartAfterA === true)
    }
    // nextRound 会清 previousLevelRank(进入新一轮)
    {
      const g = createGame({ players: [{}, {}, {}, {}], seed: 8 })
      g._state.levelRank = 14
      g._state.teamLevels = [14, 14]
      g._state.finishedOrder = [0, 2, 1, 3]
      g.applyRoundEnd()
      g.nextRound()
      const st = g._state
      assert('nextRound 后 previousLevelRank 清除(进入新 round)', st.previousLevelRank == null)
      assert('nextRound 后 isRestartAfterA 清除', st.isRestartAfterA === false)
      assert('nextRound 后 phase=playing(已发牌)', st.phase === 'playing')
    }
    // tributeInfo 在打 A 不过时返回 null
    {
      const info = E.tributeInfo([0, 2, 1, 3], [[0, 2], [1, 3]], 0, 14)
      assert('打 A 不过(levelUp=0)不贡', !info.needTribute)
    }
  }

  // ============================================================
  // === 33. Batch 1:destroy() 后 mutator 返回 game_destroyed 且不抛错 ===')
  {
    const g = createGame({ players: [{}, {}, {}, {}], seed: 1, aiPlayers: [0] })
    g.deal()
    g.destroy()
    let threw = false
    let r
    try { r = g.deal() } catch (e) { threw = true }
    assert('destroy 后 deal() 不抛错', !threw)
    assert('destroy 后 deal() 返回 game_destroyed', JSON.stringify(r) === JSON.stringify({ ok: false, error: 'game_destroyed' }))
    // playerPlay / playerPass / restartMatch / applyRoundEndFromPayload 同样返回错误对象
    assert('destroy 后 playerPlay 返回 game_destroyed', JSON.stringify(g.playerPlay(0, [g._state.hands[0][0]])) === JSON.stringify({ ok: false, error: 'game_destroyed' }))
    assert('destroy 后 restartMatch 返回 game_destroyed', JSON.stringify(g.restartMatch()) === JSON.stringify({ ok: false, error: 'game_destroyed' }))
  }

  console.log('\n=== 34. Batch 1:getState() 返回深拷贝,改返回对象不影响内部 state ===')
  {
    const g = createGame({ players: [{}, {}, {}, {}], seed: 2, aiPlayers: [] })
    g.deal()
    const st = g.getState()
    const beforeLen = g._state.hands[0].length
    st.hands[0].push({ suit: 0, rank: 99 })
    assert('改 getState().hands[0] 不影响内部 hands[0]', g._state.hands[0].length === beforeLen)
  }

  console.log('\n=== 35. Batch 1:getSnapshot() 白名单 + aiTimer 存在时不抛错 ===')
  {
    const g = createGame({ players: [{}, {}, {}, {}], seed: 3, aiPlayers: [0] })
    g.deal()
    // 强制触发 scheduleAI 产生一个活跃的 aiTimer
    g._state.currentPlayer = 0
    // scheduleAI 在 500-1000ms 后执行,这里立刻 getSnapshot 不应抛错且不含 _aiTimer
    const snap = g.getSnapshot()
    assert('getSnapshot 不抛错', typeof snap === 'object' && snap !== null)
    assert('getSnapshot 不含 _aiTimer', !('_aiTimer' in snap))
    assert('getSnapshot 含 schemaVersion:2', snap.schemaVersion === 2)
    assert('getSnapshot 含 difficulty', snap.difficulty === 'medium' || snap.difficulty === 'hard')
  }

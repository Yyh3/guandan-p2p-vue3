...
console.log('\n=== 20. v2.1 P3:网络层 selectNextHostBySeat (BC 集成,seat 优先级) ===')
{
  ...
  console.log('\n=== 31. v0.4.9 isRestartAfterA 判定 ===')
  {
    ...
    // 场景 2:打 A(14) + levelUp=0(没升级,继续打 A)→ 不算过 A
    {
      ...
      // 关键:levelUp > 0 即过 A
      g._state.finishedOrder = [1, 3, 0, 2]  // 队 1 头游
      g.applyRoundEnd()
      const st = g._state
    ...
    // 场景 3:打 2(15)→ 升 A(14)→ 不是过 A(只是升到 A)
    {
      ...
      g._state.finishedOrder = [0, 2, 1, 3]  // 头游我方,双上 +3 → 升到 12(不是 14)
      g.applyRoundEnd()
      const st = g._state
  ...
  console.log('\n=== 32.5 行为覆盖:applyPass / nextRound / applyRoundEnd 关键状态 ===')
  {
    // applyPass 跳过被放弃座位
    {
      ...
      g.deal()
      g._applySnapshot({ currentPlayer: 0, abandonedSeats: [1] })
      g.applyPass(0)
      assert('applyPass 后跳过 abandoned seat 1', g._state.currentPlayer === 2)
    }

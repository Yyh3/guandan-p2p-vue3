...
console.log('\n=== 10.10 v3.x P2-26:_applySnapshot 畸形数据防御 ===')
{
  ...
  console.log('\n=== 13. v2.1 P3:migrateHost 调整 currentPlayer (旧 host 回合时) ===')
  {
    ...
    assert('迁移后 currentPlayer 仍是 0 (新 host 接手)', game._state.currentPlayer === 0)
  }
  ...
  console.log('\n=== 14. v2.1 P3:migrateHost 调整 currentPlayer (新 host 原 seat 回合时) ===')
  {
    ...
    game.migrateHost(0, 2)
    // 迁移后 currentPlayer 应=0 (新 host 现在是 seat 0)
    assert('迁移后 currentPlayer 切到 0 (新 host)', game._state.currentPlayer === 0)
  }
...
console.log('\n=== 19.5 v2.1 P3:migrateHost 后旧座位加入 abandonedSeats,游戏可继续 ===')
  {
    const game = createGame({ seats: 4, levelRank: 5, aiPlayers: [] })
    game.deal()
    ...
    game.migrateHost(0, 2)
    const st = game._state
    assert('migrateHost 后 seat 2 在 abandonedSeats', st.abandonedSeats.includes(2))
    assert('migrateHost 后 seat 0 不在 abandonedSeats(新 host 在位)', !st.abandonedSeats.includes(0))
    // 模拟后续出牌:currentPlayer 是 1,让 1 出,然后 2(abandoned) 应被跳过
    game._applySnapshot({ currentPlayer: 1 })
    const hand1 = st.hands[1]
    game.playerPlay(1, [hand1[0]])
    // 接下来应轮到 seat 3(跳过 seat 2)
    assert('nextTurn 跳过 abandoned seat 2', game._state.currentPlayer === 3)
  }

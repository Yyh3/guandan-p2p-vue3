<template>
  <div class="page" :class="{ dealing: isDealing, bomb: isShaking, 'is-landscape': isLandscape, 'simple-mode': simpleMode }">
    ...
    <!-- ★ v0.4.29 精简:结算战绩卡改用共享 ResultOverlay 组件(与移动端同一份模板/样式) -->
    <ResultOverlay
      :phase="phase"
      :is-restart-after-a="isRestartAfterA"
      :level-up="levelUp"
      :next-level-label="nextLevelLabel"
      :finished-order="finishedOrder"
      :my-win="myWin"
      :double-up="doubleUp"
      :is-p2-p-mode="isP2PMode"
      :is-network-host="isNetworkHost"
      :player-name="playerName"
      :rank-color="rankColor"
      :is-winning-seat="isWinningSeat"
      @back="onBack"
      @primary="onPrimaryResultAction"
      @back-to-room="onBackToRoom"
    />
...
// ★ v0.4.21 对抗性审查 V0420-3 修复 + Phase3 UI 修复:
//   onHostLost 必须定义在 onMounted 外部,onUnmounted 才能拿到引用精确 off;
//   之前 const onHostLost 在 onMounted 内部,卸载时访问不到导致 listener 泄漏。
//   同时补充 import useRoute,否则 route 未定义。
...
const {
  ...
  onPrimaryResultAction,  // ★ v0.4.29:结算卡「下一局/重开一轮」统一回调
...
/* ★ v0.4.29:结算战绩卡样式已迁移至 ResultOverlay.vue(桌面/移动端共享) */

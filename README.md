...
**小计**:**40 测试套件 / 1947 用例 / 0 失败**(v0.4.0 的 862 → 972 → 1609 → 1675 → 1781 → 1837 → 1857 → 1859 → 1889(v0.4.18 实测 + v0416-adversarial-fixes 30 case),含 v0.4.13 对抗性审查 34 case + v0.4.14 对抗性复查 50 case + v0.4.15 边缘防御 2 case + v0416-adversarial-fixes 30 case)
...
# 测试
npm test                  # 全部 40 套件 / 1947 用例(v0.4.18)
...
- **测试覆盖**:1947 单测(v0.3.0 的 701 → v0.4.0 862 → v0.4.9 1609 → v0.4.10 1675 → v0.4.11 1781 → v0.4.12 1837 → v0.4.14 1857 → v0.4.15 1859 → v0.4.18 1889,含 v0.4.13 对抗性审查 34 case + v0.4.14 对抗性复查 50 case + v0.4.15 边缘防御 2 case + v0416-adversarial-fixes 30 case),改算法必加测试
...
- 详细文档:[docs/](./docs/)
- 当前版本:`v0.4.18`(2026-06-29,V0414-04 本地选举协议最小修复 — `network.js requestPromoteToHost()` 扩展本地 self-loop 到所有 transport(不只 BC),WS / AndroidWs 模式下旧 host transport 死了 joiner 也能本地升级;`rebuildAsHost()` 失败分支(`_createTransport` / `open('self')`)emit `'host:lost'` 业务事件 + `useGameLogic.onHostMigrated` 的 rebuildAsHost promise.catch 也 emit host:lost,浏览器 ws joiner 无 server 能力走 host:lost → 跳首页;AndroidWs native / Node ws joiner 走 rebuildAsHost 成功 → 起新 server → broadcast TRANSPORT_REBUILD_ANNOUNCE;已知未做留 v0.4.19+:确定性本地选举 / canHost + hostAddress 上报 / mDNS / UDP 第二发现通道;40 套件 / 1947/0 单测,JDK 21)

---

**最后更新**:2026-06-29(v0.4.18 发布:V0414-04 本地选举协议最小修复——`requestPromoteToHost()` 扩展本地 self-loop 到所有 transport(不只 BC),让 WS / AndroidWs joiner 端 host 真的崩溃时也能本地升级 + `rebuildAsHost()` 失败 emit `'host:lost'` + `useGameLogic.onHostMigrated` rebuildAsHost promise.catch 也 emit host:lost;浏览器 ws joiner 无 server 能力走 host:lost → 跳首页让用户重连;AndroidWs native / Node ws joiner 走 rebuildAsHost 成功起新 server → broadcast TRANSPORT_REBUILD_ANNOUNCE 让其他 joiner 重连;已知未做留 v0.4.19+:确定性本地选举 / canHost + hostAddress 上报 / mDNS / UDP 第二发现通道;40 套件 / 1947 单测全过)

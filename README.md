...
# 测试
npm test                  # 全部 42 套件 / 1916 用例(v0.4.21)
...
- **测试覆盖**:1916 单测(v0.3.0 的 701 → v0.4.0 862 → v0.4.9 1609 → v0.4.10 1675 → v0.4.11 1781 → v0.4.12 1837 → v0.4.14 1857 → v0.4.15 1859 → v0.4.18 1889 → v0.4.20 1891 → v0.4.21 1916,含 v0.4.13 对抗性审查 34 case + v0.4.14 对抗性复查 52 case + v0.4.15 边缘防御 19 case + v0416-adversarial-fixes 30 case + v0417-adversarial-fixes 38 case + v0418-adversarial-fixes 20 case + v0419-adversarial-fixes 36 case + v0420-adversarial-fixes 35 case + v0421-adversarial-fixes 25 case),改算法必加测试
...
| v0.4.21 | ✅ 当前 | V0421 对抗性审查 4 个 BUG 修复 — `smartReconnectToPeers` 精确 off(不传 fn 不再清空所有 connect 监听器,BUG-V0420-1 严重)+ setTimeout 4 处都加 clearTimeout(内存泄漏修复,BUG-V0420-2)+ `GameViewDesktop.onUnmounted` 改用命名函数 onHostLost + 精确 off(BUG-V0420-3)+ `findMinStraightFlush` 允许 A(14)同花顺(BUG-AI-1);42 套件 / 1916/0 单测 |
...
- 详细文档:[docs/](./docs/)
- 当前版本:`v0.4.21`(2026-06-30,V0421 对抗性审查 4 个 BUG 修复 — `smartReconnectToPeers` 精确 off + setTimeout 4 处 clearTimeout + `GameViewDesktop.onUnmounted` 命名函数 + 精确 off + `findMinStraightFlush` 允许 A 同花顺;42 套件 / 1916/0 单测,JDK 21)

---

**最后更新**:2026-06-30(v0.4.21 发布:V0421 对抗性审查 4 个 BUG 修复 — `smartReconnectToPeers` 改用精确 `off('connect', onConnect)` + `off('error', onError)` 不再清空所有 connect 监听器(BUG-V0420-1 严重);4 处 setTimeout 都加 `clearTimeout(timer); timer=null` 避免泄漏(BUG-V0420-2);`GameViewDesktop.onUnmounted` 用命名函数 `onHostLost` + 精确 off(BUG-V0420-3);`findMinStraightFlush` 改 `r >= 3 && r <= 14` 允许 A(14)同花顺(BUG-AI-1);42 套件 / 1916 单测全过)

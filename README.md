...
**小计**:**41 测试套件 / 1985 用例 / 0 失败**(v0.4.0 的 862 → 972 → 1609 → 1675 → 1781 → 1837 → 1857 → 1859 → 1889(v0.4.19 实测 + v0416-adversarial-fixes 30 case),含 v0.4.13 对抗性审查 34 case + v0.4.14 对抗性复查 50 case + v0.4.15 边缘防御 2 case + v0416-adversarial-fixes 30 case)
...
# 测试
npm test                  # 全部 41 套件 / 1985 用例(v0.4.19)
...
- **测试覆盖**:1985 单测(v0.3.0 的 701 → v0.4.0 862 → v0.4.9 1609 → v0.4.10 1675 → v0.4.11 1781 → v0.4.12 1837 → v0.4.14 1857 → v0.4.15 1859 → v0.4.19 1889,含 v0.4.13 对抗性审查 34 case + v0.4.14 对抗性复查 50 case + v0.4.15 边缘防御 2 case + v0416-adversarial-fixes 30 case),改算法必加测试
...
| v0.4.19 | ✅ 当前 | V0419 follow-up 4 项 — `network.js selectNextHostCandidate()` 确定性选举(UUID 字典序 + canHost 过滤,v2.1 旧版重命名 selectNextHostBySeat 保留)+ `selfInfo` 加 `canHost` + `hostAddress` 字段(V0419-02 peer:join 上报能力)+ `broadcastPeerLeave` payload 加 `newHostAddress`(V0419-03 自动从 peers 提取)+ `close({broadcast:true, newHostSeat, newHostAddress})` 关闭前广播完整新 host 信息(V0419-04 简化 TOMBSTONE)。`requestPromoteToHost` 集成新选举 + `canHostAsNewHost()` 守卫:浏览器 ws joiner 走 host:lost 跳首页;AndroidWs native / Node ws joiner 走 rebuildAsHost 起新 server。已知未做留 v0.4.20+:mDNS / UDP 广播 / 固定服务等真正第二发现通道(scope 大需 native),41 套件 / 1985/0 单测 |
...
- 详细文档:[docs/](./docs/)
- 当前版本:`v0.4.19`(2026-06-30,V0419 follow-up 4 项 — `network.js selectNextHostCandidate()` 确定性选举(UUID 字典序 + canHost 过滤,v2.1 旧版重命名 selectNextHostBySeat 保留)+ `selfInfo` 加 `canHost` + `hostAddress` 字段(V0419-02 peer:join 上报能力)+ `broadcastPeerLeave` payload 加 `newHostAddress`(V0419-03 自动从 peers 提取)+ `close({broadcast:true, newHostSeat, newHostAddress})` 关闭前广播完整新 host 信息(V0419-04 简化 TOMBSTONE)。`requestPromoteToHost` 集成新选举 + `canHostAsNewHost()` 守卫:浏览器 ws joiner 走 host:lost 跳首页;AndroidWs native / Node ws joiner 走 rebuildAsHost 起新 server。已知未做留 v0.4.20+:mDNS / UDP 广播 / 固定服务等真正第二发现通道(scope 大需 native);41 套件 / 1985/0 单测,JDK 21)

---

**最后更新**:2026-06-30(v0.4.19 发布:V0419 follow-up 4 项 — `selectNextHostCandidate` 确定性 UUID 字典序选举 + `selfInfo` 加 `canHost` + `hostAddress` 让新 host 选择更优 + `broadcastPeerLeave` payload 自动提取 `newHostAddress` + `close({broadcast:true})` 关闭前广播完整新 host 信息;`requestPromoteToHost` 集成新选举 + `canHostAsNewHost()` 守卫浏览器 ws joiner 走 host:lost;AndroidWs native / Node ws joiner 走 rebuildAsHost 起新 server;已知未做留 v0.4.20+:mDNS / UDP 广播 / 固定服务等真正第二发现通道;41 套件 / 1985 单测全过)

...
# 3. 跑测试(可选,验证环境)
npm test
# 期望: 50 套件 / 2302 用例 / 0 失败 (v0.4.21+ / Phase 2 基线)
```
...
- ✅ **AI 单机对战**:3 个 AI + 1 个玩家,三档难度(Easy / Medium / Hard)
...
- ✅ **真机 WebSocket/AndroidWs host 迁移(v0.4.17 - v0.4.20)**:host 主动退出广播 PEER_LEAVE + TRANSPORT_REBUILD_ANNOUNCE → joiner 立即收到 newHostAddress 走 N-3 兜底(v0.4.17);host 崩溃场景通过 `smartReconnectToPeers` 循环 try-connect localStorage 缓存的 peer hostAddress 找新 host(v0.4.20,1 小时 TTL,跨 session 持久化);AndroidWs native / Node ws joiner 还能 `rebuildAsHost` 自己起新 server(v0.4.18 兜底)。已知未做留 v0.4.21+:真 mDNS(Capacitor plugin)/ UDP 广播(原生层)/ 固定服务 discovery server(scope 大需 native)
...
- ✅ **完整工程化**:2374 单元测试(50 套件 / 0 失败,+1436 用例,BUG-001~008 + BUG-RC3 + v0.4.9 新功能 + v0.4.10 v049 9 bug + v0.4.11 v0410 8 bug + v0.4.12 P2P 回归 56 case + v0.4.13 对抗性审查 34 case + v0.4.14 对抗性复查 50 case + v0.4.15 边缘防御 2 case + v0416-adversarial-fixes 30 case + Phase1 引擎/AI/状态机补强 + Phase2 网络层补强 + Phase4 UI/useGameLogic 与 host 迁移连续对局回归 + P0 收尾 hostEpoch/WS E2E/dev hook 收敛)+ Chrome CDP 4-tab 联机回归 + Playwright 9 E2E(含 WS 跨设备)
...
**小计**:**50 测试套件 / 2302 用例 / 0 失败**(v0.4.0 的 862 → 972 → 1609 → 1675 → 1781 → 1837 → 1857 → 1889(v0.4.18 实测) → 1891(v0.4.20 实测) → 1916(v0.4.21 实测 + v0421-adversarial-fixes 25 case) → 2156(Phase1-4 补强,新增 network-phase2 25 + useGameLogic 22 + host-migration-consecutive 20 + 引擎/AI/状态机/网络层回归 case) → 2302(Phase 3 UI polish) → 2302(Phase 2 收尾),含 v0.4.13 对抗性审查 34 case + v0.4.14 对抗性复查 52 case + v0.4.15 边缘防御 19 case + v0416-adversarial-fixes 30 case + v0417-adversarial-fixes 38 case + v0418-adversarial-fixes 20 case + v0419-adversarial-fixes 36 case + v0420-adversarial-fixes 35 case + v0421-adversarial-fixes 25 case)
...
# 测试
npm test                  # 全部 50 套件 / 2302 用例(v0.4.21+ / Phase 4 基线)
npm run e2e               # Playwright E2E,8 个核心主流程场景全绿
...
5. **AI 三档难度(easy / medium / hard)**(规则 + 贪心搜索,**非**深度学习)
...
- **测试覆盖**:2302 单测(v0.3.0 的 701 → v0.4.0 862 → v0.4.9 1609 → v0.4.10 1675 → v0.4.11 1781 → v0.4.12 1837 → v0.4.14 1857 → v0.4.15 1859 → v0.4.18 1889 → v0.4.20 1891 → v0.4.21 1916 → 2198(v0.4.21+ 对抗性审查后续修复 + P0/P1 止血修复) → 2302(Phase 3 UI polish) → 2302(Phase 2 收尾)),含 v0.4.13 对抗性审查 34 case + v0.4.14 对抗性复查 52 case + v0.4.15 边缘防御 19 case + v0416-adversarial-fixes 30 case + v0417-adversarial-fixes 38 case + v0418-adversarial-fixes 20 case + v0419-adversarial-fixes 36 case + v0420-adversarial-fixes 35 case + v0421-adversarial-fixes 25 case),改算法必加测试
...
- 详细文档:[docs/](./docs/)
- 当前版本:`v0.4.21`(2026-07-14,V0421 对抗性审查 4 个 BUG 修复 + P0/P1 止血 — `smartReconnectToPeers` 精确 off + setTimeout 4 处 clearTimeout + `GameViewDesktop.onUnmounted` 命名函数 + 精确 off + `findMinStraightFlush` 允许 A 同花顺 + host 迁移保留旧 host 座位与手牌 + READY 走 host 权威提交 + 浏览器 host 不再展示虚假跨设备邀请;50 套件 / 2302/0 单测,JDK 21)

---

**最后更新**:2026-07-14(v0.4.21+ 发布:V0421 对抗性审查 4 个 BUG 修复 + 后续 P0/P1 止血 — `smartReconnectToPeers` 改用精确 `off('connect', onConnect)` + `off('error', onError)` 不再清空所有 connect 监听器(BUG-V0420-1 严重);4 处 setTimeout 都加 `clearTimeout(timer); timer=null` 避免泄漏(BUG-V0420-2);`GameViewDesktop.onUnmounted` 用命名函数 `onHostLost` + 精确 off(BUG-V0420-3);`findMinStraightFlush` 改 `r >= 3 && r <= 14` 允许 A(14)同花顺(BUG-AI-1);host 迁移保留旧 host 座位与手牌避免四人排名缺失;READY 走 host 权威提交保证多客户端一致;浏览器 host 不再展示虚假跨设备邀请;50 套件 / 2302 单测全过)

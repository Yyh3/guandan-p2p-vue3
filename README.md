...
**小计**:**39 测试套件 / 1927 用例 / 0 失败**(v0.4.0 的 862 → 972 → 1609 → 1675 → 1781 → 1837 → 1857 → 1859 → 1889(v0.4.17 实测 + v0416-adversarial-fixes 30 case),含 v0.4.13 对抗性审查 34 case + v0.4.14 对抗性复查 50 case + v0.4.15 边缘防御 2 case + v0416-adversarial-fixes 30 case)
...
# 测试
npm test                  # 全部 39 套件 / 1927 用例(v0.4.17)
...
- **测试覆盖**:1927 单测(v0.3.0 的 701 → v0.4.0 862 → v0.4.9 1609 → v0.4.10 1675 → v0.4.11 1781 → v0.4.12 1837 → v0.4.14 1857 → v0.4.15 1859 → v0.4.17 1889,含 v0.4.13 对抗性审查 34 case + v0.4.14 对抗性复查 50 case + v0.4.15 边缘防御 2 case + v0416-adversarial-fixes 30 case),改算法必加测试
...
- 详细文档:[docs/](./docs/)
- 当前版本:`v0.4.17`(2026-06-29,v0.4.16 对抗性复查 5 项 V0416 真 bug 修复 + 1 项误报澄清 — V0416-02 network.js `rebuildAsHost()` 顺序修复 — 先起新 server + 用**旧 transport 引用**广播 TRANSPORT_REBUILD_ANNOUNCE + 再 close 旧 transport,让 WS/AndroidWs 真机 host 迁移真正闭环(旧版 close 旧 transport → sendMessage 用新 transport 但无 client → joiner 永远收不到新 host 地址);V0416-03 RoomView `onNet('peer:leave')` 检测 `seat===0` 跳首页 + 提示"房主已退出,房间解散";V0416-04 网络层 `_DISCONNECT payload.seat===-1` (joiner 端 ws.onclose) → emit `'host:lost'` 业务事件 + GameViewDesktop 监听跳首页;V0416-06 README 测试数字统一;V0416-05 误报澄清;V0416-01 修复未合入 master 留 follow-up;39 套件 / 1927/0 单测,JDK 21)

---

**最后更新**:2026-06-29(v0.4.17 发布:v0.4.16 对抗性复查 5 项 V0416 真 bug 修复 + 1 项误报澄清——V0416-02 `rebuildAsHost()` 关键顺序修复(先起新 server + 用旧 transport 引用广播 + 再 close 旧 transport),让 WS/AndroidWs 真机 host 迁移真正闭环;V0416-03 RoomView `onNet('peer:leave')` 检测 `seat===0` 跳首页 + 提示"房主已退出,房间解散";V0416-04 网络层 `_DISCONNECT payload.seat===-1` emit `'host:lost'` + GameViewDesktop 监听跳首页 + onUnmounted 清理;V0416-06 README 测试数字统一(删除 1887 作当前基线描述);V0416-05 误报澄清(RoomView 实际用 emoji 🟢/🔴 不是空字符串);V0416-01 修复未合入 master 留 follow-up;39 套件 / 1927 单测全过)

...
# 3. 跑测试(可选,验证环境)
npm test
# 期望: 61 套件 / 2724 用例 / 0 失败 (v0.4.25 基线)
```
...
- ✅ **AI 单机对战**:3 个 AI + 1 个玩家,两档难度(Medium / Hard,规则+贪心;v0.4.25 起 AI 开局/中局保留炸弹,残局才抢牌权,单张不拆炸弹)
...
- ✅ **真机 WebSocket/AndroidWs host 迁移(v0.4.17 - v0.4.22)**:host 主动退出广播 PEER_LEAVE + TRANSPORT_REBUILD_ANNOUNCE → joiner 立即收到 newHostAddress 走 N-3 兜底(v0.4.17);host 崩溃场景通过 `smartReconnectToPeers` 循环 try-connect localStorage 缓存的 peer hostAddress 找新 host(v0.4.20,1 小时 TTL,跨 session 持久化);AndroidWs native / Node ws joiner 还能 `rebuildAsHost` 自己起新 server(v0.4.18 兜底);v0.4.22 修复 host 主动迁移时完整四家手牌泄露给所有 joiner 的问题,完整 snapshot 只定向发给新 host。
...
- ✅ **JDK 21 环境**:Capacitor 8 / AGP 8.13 要求;本机 Windows 工具链全部国内镜像安装(JDK 21 `C:\dev\jdk-21` / Android SDK `C:\dev\android-sdk` / Gradle `C:\dev\gradle-8.14.3`,依赖走阿里云镜像,见 AGENTS.md「构建环境」)
- ✅ **零公网依赖**:无后端 / 无 fetch / 无远程字体 / 无 CDN
- ✅ **零第三方 UI 库**:Vue 3 + 原生 CSS + 自写设计 token
- ✅ **零账号系统**:本地 localStorage 存昵称 / 头像 / 设置 / 战绩
- ✅ **完整工程化**:2724 单元测试(61 套件 / 0 失败,含 v0.4.25 审查修复 + UX 六件套 + 理牌三模式新增回归)+ Chrome CDP 4-tab 联机回归 + Playwright E2E(含 WS 跨设备)
- ✅ **零依赖运行时**:核心算法 100% 自实现(`src/common/` 纯 JS,无 Vue 依赖)
- ✅ **P2P 同步修复 v0.4.x(2026-06-27)**:BUG-001~008 共 8 个 P2P 联机连环 bug 已修复,详见 §3.3 / §3.4
- ✅ **身份安全(v0.4.25)**:座位重连须 `uuid + resumeToken` 双凭证(host 签发,仅定向下发),杜绝仅凭 uuid 冒名占座;snapshot 四手牌 card id 全局唯一校验;迁移候选统一确定性选举
- ✅ **对局体验(v0.4.25)**:选中牌型实时预览(牌型+可压指示)/ 拖动连选 / 记牌器 / 出牌归属+牌型名标签 / 飞牌轨迹按座位方向 / 特殊牌型四色弹跳特效 + 中文语音播报 / 真爆炸音效 / 级牌进度轨 / 简洁模式
- ✅ **理牌三模式(v0.4.25)**:点数分组 / 牌型优先(同花顺/顺子/连对/钢板/炸弹各自成列,默认靠左)/ 自定义(拖动单牌手动排序)

### 3.2 v0.4.10 后待办(等 v4.0)

- ❌ **iOS 脚手架**:Capacitor iOS 工程未建,Mac + Xcode 走一遍
- ❌ **多语言**:当前中文 UI 硬编码,英文/繁体未抽 i18n
- ❌ **弱网 / 隧道 / 高铁压测数据**:当前只在高速局域网 + WiFi 测过
- ✅ **移动端响应式**:v0.4.10 文档化(`GameViewMobile` 双布局竖屏+横屏兜底,详见 CHANGELOG)
...
| 2 | selfSeat 手牌 | 4 client 同 seed,`myHand === sortGrouped(hands[selfSeat])` | `src/views/game/finish-deal-seat.test.js` | 57 | ✅ 通过 |
...
| 6 | WS/BC transport 稳定字段与定向路由 | transport.type / IPv6 URL / outbox 保留 msg.to / reconnect 计数器 / SEAT_SWAP_ACK relay 等 | `src/common/network-phase2.test.js` | 25 | ✅ 通过 |
| 7 | useGameLogic 生命周期与状态 | selfSeat getter、timer 清理、AI takeover 过期 state、重开一局单一动画源 | `src/views/game/useGameLogic.test.js` | 27 | ✅ 通过 |
| 8 | host 迁移后连续对局 | BC 下原 host 让位给 seat 2,新 host 上任后仍能广播,旁观者收到 | `src/common/network-host-migration-consecutive.test.js` | 20 | ✅ 通过 |
| 9 | **端到端 4 真机** | 1 真机 host + 1 真机 joiner + 2 浏览器 tab(v2.2,2026-06-13) | 实地联机 + 截图存档 | — | ✅ 已通过(v0.3.0) |
| 7 | **端到端 4-tab** | Chrome CDP 4-tab 局域网联机 demo(v3.8,2026-06-10) | Playwright E2E + 截图 | — | ✅ 已通过(v0.2.0) |

**小计**:**61 测试套件 / 2724 用例 / 0 失败**(v0.4.25 基线)
...
# 测试
npm test                  # 全部 61 套件 / 2724 用例(v0.4.25 基线)
npm run e2e               # Playwright E2E,12 个核心主流程场景全绿
npm run test:engine       # 规则引擎 158 用例
npm run test:ai           # AI 80 用例
npm run test:game         # 状态机 184 用例
npm run test:anim         # 发牌动画 + 音频 160 用例
npm run test:ws           # WsServer + 跨设备 30 用例(v2.0/v2.2)
...
node src/views/game/finish-deal-seat.test.js  # selfSeat 手牌 57 case(BUG-002)
...
5. **AI 两档难度(medium / hard)**(规则 + 贪心搜索,**非**深度学习;hard = 防守优先 + 炸弹保留)
...
- **测试覆盖**:2724 单测(61 套件 / 0 失败,v0.4.25 基线),改算法必加测试
- **设计 token 化**:`src/styles/tokens.css` 集中管理,改色改间距改这里
- **响应式**:桌面 1280×800 + 移动端竖屏/横屏双布局
...
| v0.4.8 | ✅ 完成 | P2P 同步修复 8 BUG + host 迁移 game/UI 闭环 + 真实 BGM(7 首 MP3),21 套件 / 972/0 单测 |
| v0.4.9 | ✅ 完成 | AI 难度分档 + 二维码真扫码 + 战绩趋势图 + 真实 SFX + 过 A 重开 + UI 主题刷新,32 套件 / 1609/0 单测 |
| v0.4.10 | ✅ 完成 | 移动端响应式文档化 + v0.4.9 静态审查 9 bug 修复(diff 未定义 / isRestartAfterA 贯通 / MATCH_RESTART seed / relay 白名单 / 真实音效 fallback / setSettings 合并当前值 / IP 端口校验),33 套件 / 1675/0 单测 |
| v0.4.11 | ✅ 完成 | v0.4.10 静态审查 8 bug 修复(ROUND_END host-only / 过 A 落盘 / MATCH_RESTART 鉴权 / host UI refresh / 音效 unlock 清理 / bgmStyle+sfxMode 入场 / scheduleAI difficulty / 版本号动态化),34 套件 / 1781/0 单测 |
| v0.4.12 | ✅ 完成 | v0.4.11 修复后回归测试补充(P2P 端到端 56 case 覆盖 ROUND_END host-only / MATCH_RESTART 鉴权去重 phase gate / applyRoundEndFromPayload 幂等 / 同 seed restartMatch 等关键路径),35 套件 / 1837/0 单测 |
| v0.4.13 | ✅ 完成 | v0.4.12 对抗性审查 8 项 P0/P1/P2 bug 修复(network canBroadcast + broadcastPeerLeave + close broadcast / createGame.destroy / onP2PStateSnapshot 走 refreshUiFromGameState / migrateHost emit turn / onP2PRoundEnd 去重 / onP2PPeerJoin 单一路径 / onP2PPlay ts 去重 + applyPlay 防御),35 套件 / 1837/0 单测 |
| v0.4.14 | ✅ 完成 | v0.4.12 对抗性复查 6 项 V0412 bug 修复(migrateHost abandonedSeats 不 push 0 / AI pass 广播 / _applySnapshot 补 isRestartAfterA 等 / 新增 game.getSnapshot 单一来源 / onNext P2P 非 host 不动 phase),37 套件 / 1857/0 单测 |
| v0.4.15 | ✅ 完成 | 对抗性复查 3 项瑕疵修复(V0412-04 lastAppliedRoundId `!== undefined` 边缘防御 / CHANGELOG & commit message 基线数字 1887 → 实测 1857 / BUILD.md & README 加 `npm install` 必跑提醒),37 套件 / 1859/0 单测 |
| v0.4.16 | ✅ 完成 | v0.4.14 对抗性复查 5 项 V0414 真 bug 修复(V0414-01 RoomView→GameView 跳转 URL 加 role + GameView isP2PMode 兼容 role=host / V0414-02 useGameLogic.onHostMigrated 末尾 fire-and-forget 调 net.rebuildAsHost / V0414-03 RoomView showMenu host 端 net.close({broadcast:true}) / V0414-05 broadcastPeerLeave snapshot 超 64KB fallback 保留 newHostSeat / V0414-06 误报澄清),V0414-04 留 v0.4.20 follow-up,38 套件 / 1889/0 单测 |
| v0.4.17 | ✅ 完成 | v0.4.16 对抗性复查 5 项 V0416 真 bug 修复 + 1 项误报澄清(V0416-02 rebuildAsHost 关键顺序修复 — 先起新 server + 用旧 transport 引用广播 TRANSPORT_REBUILD_ANNOUNCE + 再 close 旧 transport,让 WS/AndroidWs 真机 host 迁移真正闭环 / V0416-03 RoomView onNet('peer:leave') 检测 seat=0 跳首页 + 提示"房主已退出,房间解散" / V0416-04 网络层 _DISCONNECT payload.seat=-1 emit 'host:lost' + GameViewDesktop 监听跳首页 / V0416-06 README 测试数字统一 / V0416-05 误报澄清);V0416-01 修复未合入 master 留 follow-up,39 套件 / 1927/0 单测 |
| v0.4.18 | ✅ 完成 | V0414-04 本地选举协议最小修复 — `network.js requestPromoteToHost()` 扩展本地 self-loop 到所有 transport(不只 BC)+ `rebuildAsHost()` 失败分支 emit `'host:lost'` + `useGameLogic.onHostMigrated` 的 rebuildAsHost promise.catch 也 emit host:lost。浏览器 ws joiner 无 server 能力走 host:lost → 跳首页;AndroidWs native / Node ws joiner 走 rebuildAsHost 成功 → 起新 server → broadcast TRANSPORT_REBUILD_ANNOUNCE。已知未做(留 v0.4.19+):确定性本地选举 / canHost + hostAddress 上报 / mDNS / UDP 第二发现通道,40 套件 / 1947/0 单测 |
| v0.4.19 | ✅ 完成 | V0419 follow-up 4 项 — `network.js selectNextHostCandidate()` 确定性选举(UUID 字典序 + canHost 过滤,v2.1 旧版重命名 selectNextHostBySeat 保留)+ `selfInfo` 加 `canHost` + `hostAddress` 字段(V0419-02 peer:join 上报能力)+ `broadcastPeerLeave` payload 加 `newHostAddress`(V0419-03 自动从 peers 提取)+ `close({broadcast:true, newHostSeat, newHostAddress})` 关闭前广播完整新 host 信息(V0419-04 简化 TOMBSTONE)。`requestPromoteToHost` 集成新选举 + `canHostAsNewHost()` 守卫:浏览器 ws joiner 走 host:lost 跳首页;AndroidWs native / Node ws joiner 走 rebuildAsHost 起新 server,41 套件 / 1985/0 单测 |
...
| v0.4.21 | ✅ 完成 | V0421 对抗性审查 4 个 BUG 修复 + P0/P1 止血 — `smartReconnectToPeers` 精确 off(不传 fn 不再清空所有 connect 监听器,BUG-V0420-1 严重)+ setTimeout 4 处都加 clearTimeout(内存泄漏修复,BUG-V0420-2)+ `GameViewDesktop.onUnmounted` 改用命名函数 onHostLost + 精确 off(BUG-V0420-3)+ `findMinStraightFlush` 允许 A(14)同花顺(BUG-AI-1)+ host 迁移保留旧 host 座位与手牌 + READY 走 host 权威提交 + 浏览器 host 不再展示虚假跨设备邀请;50 套件 / 2302/0 单测 |
| v0.4.22 | ✅ 完成 | 对抗性审查 P0-P1-横屏修复 — 修复 host 主动迁移泄露完整四家手牌(P0-06)、WebSocket ROOM_PROBE 定向回复(P1-13)、浏览器扫描到房间后按真实 IP:port 加入(P1-11)、transport 层 host seat 硬编码(P0-08)、手机横屏操作区/字号/安全区/手牌全显/预选等体验问题;52 套件 / 约 2400/0 单测 |
| v0.4.24 | ✅ 完成 | 四路并行对抗性审查修复 + UI/HCI — GameViewDesktop 补 haptics 导入、host 主动退出迁移改 getSnapshot + close 迁移字段、JoinView parseHostAddress 修复、play 事件数字 type 映射牌型名(炸弹特效/语音复活)、smartReconnect 三重修复、战绩按 mySeat 统计、桌面 Esc 菜单、CHAT_QUICK 跨端聊天;59 套件 / 2106/0 单测 |
| v0.4.25 | ✅ 当前 | 对抗性审查第一阶段(resumeToken 双凭证防冒名 + 迁移角色以 network 为准 + seat 0 地址缓存 + snapshot 全局牌 id 唯一 + 候选选举统一 + safe-area 修复)+ 主流掼蛋手游 UX 六件套(选中预览/拖动连选/记牌器/飞牌轨迹/简洁模式/级牌进度轨)+ 理牌三模式(点数/牌型/自定义)+ AI 开局不再扔炸弹 + 手牌布局三调(去点数角标/大牌靠左/重叠收紧)+ 经典扑克大小王;61 套件 / 2724/0 单测 |
| v4.0 | 💭 构思 | 录像回放 + iOS 脚手架 + 弱网压测 |
...
- 详细文档:[docs/](./docs/)
- 当前版本:`v0.4.25`(2026-07-19,对抗性审查第一阶段(resumeToken 防冒名等)+ UX 六件套(选中预览/拖动连选/记牌器/飞牌轨迹/简洁模式/级牌进度轨)+ 理牌三模式 + AI 开局不扔炸弹 + 手牌布局三调;61 套件 / 2724 用例全绿,JDK 21)

---

**最后更新**:2026-07-19(v0.4.25:座位重连须 uuid+resumeToken 防冒名;RoomView 角色以 network 为准;seat 0 hostAddress 缓存;snapshot 全局牌 id 唯一;迁移候选统一 selectNextHostCandidate;横屏 safe-area 左右修复;选中牌型实时预览;拖动连选;记牌器;出牌归属+牌型名标签;飞牌轨迹按座位方向;特殊牌型四色弹跳特效+中文语音播报;真爆炸音效;级牌进度轨;简洁模式;理牌三模式(点数/牌型/自定义);AI 开局/中局保留炸弹不拆炸;手牌去点数角标、大牌型靠左、列重叠收紧;经典扑克大小王(竖排 JOKER+卡通小丑 SVG);61 套件 / 2724 用例全绿)

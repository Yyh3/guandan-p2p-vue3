...
# 3. 跑测试(可选,验证环境)
npm test
# 期望: 56 套件 / 约 1965 用例 / 0 失败 (v0.4.22+ / 修复报告收尾基线)
```
...
- ✅ **完整工程化**:约 2400 单元测试(52 套件 / 0 失败,含 v0.4.22 P0-P1-横屏修复新增回归)+ Chrome CDP 4-tab 联机回归 + Playwright 9 E2E(含 WS 跨设备)
...
**小计**:**56 测试套件 / 约 2000 用例 / 0 失败**(v0.4.22 阶段二 P1 收尾:joiner 优雅离开、返回房间复用 session、QR 编码可打开的加入 URL、host 迁移 ACK;含 v0416-adversarial-fixes 31 case + v0417-adversarial-fixes 38 case + v0418-adversarial-fixes 20 case + v0419-adversarial-fixes 36 case + v0420-adversarial-fixes 35 case + v0421-adversarial-fixes 29 case + v0422-adversarial-fixes 19 case + v0423-adversarial-fixes 10 case + network-graceful-leave 17 case)
...
# 测试
npm test                  # 全部 56 套件 / 约 1965 用例(v0.4.22+ / 修复报告收尾基线)
...
- **测试覆盖**:约 2000 单测(56 套件 / 1965 case / 0 失败,v0.4.22+ 修复报告收尾基线;含 v0416-adversarial-fixes 31 case + v0417-adversarial-fixes 38 case + v0418-adversarial-fixes 20 case + v0419-adversarial-fixes 36 case + v0420-adversarial-fixes 35 case + v0421-adversarial-fixes 29 case + v0422-adversarial-fixes 19 case + v0423-adversarial-fixes 10 case + network-graceful-leave 17 case),改算法必加测试
...
| v0.4.22 | ✅ 当前 | 对抗性审查 P0-P1-横屏修复 — 修复 host 主动迁移泄露完整四家手牌(P0-06)、WebSocket ROOM_PROBE 定向回复(P1-13)、浏览器扫描到房间后按真实 IP:port 加入(P1-11)、transport 层 host seat 硬编码(P0-08)、手机横屏操作区/字号/安全区/手牌全显/预选等体验问题;52 套件 / 约 2400/0 单测 |
...
- 详细文档:[docs/](./docs/)
- 当前版本:`v0.4.22`(2026-07-16,v0.4.22 对抗性审查 P0-P1-横屏修复 — host 主动迁移不泄露完整手牌、ROOM_PROBE 定向回复、浏览器扫描按真实 IP 加入、transport host seat 硬编码修复、手机横屏操作区/字号/安全区/手牌全显/预选优化;52 套件 / 约 2400/0 单测,JDK 21)

---

**最后更新**:2026-07-16(v0.4.22 发布:对抗性审查 P0-P1-横屏修复 — host 主动迁移时完整四家手牌 snapshot 只定向发给新 host,旁观者不再收到完整手牌;WebSocket ROOM_PROBE 直接对探测 socket 回复,避免广播打扰已连接玩家;浏览器扫描到局域网房间后按真实 IP:port 加入而非走本地 BC;transport 层 forceDisconnectSeat 等 host-only 广播的 from 字段跟随当前 hostSeat,不再硬编码 0;手机横屏操作栏改为 3 列、触控目标 ≥44px、字号提升、增加左右安全区、房间页横屏布局防重叠、handOverlap 监听 viewport 变化、手牌显示全部实体牌而非仅前 3 张、允许非本人回合预选和理牌;52 套件 / 约 2400 单测全过)

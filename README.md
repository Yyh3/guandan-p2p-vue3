...
**小计**:**37 测试套件 / 1859 用例 / 0 失败**(v0.4.0 的 862 → 972 → 1609 → 1675 → 1781 → 1837 → 1857 → 1859(v0.4.15 实测 + 边缘防御 2 case),含 v0.4.13 对抗性审查 34 case + v0.4.14 对抗性复查 50 case + v0.4.15 边缘防御 2 case)
...
# 测试
npm test                  # 全部 37 套件 / 1859 用例(v0.4.15)
...
- **测试覆盖**:1859 单测(v0.3.0 的 701 → v0.4.0 862 → v0.4.9 1609 → v0.4.10 1675 → v0.4.11 1781 → v0.4.12 1837 → v0.4.14 1857 → v0.4.15 1859,含 v0.4.13 对抗性审查 34 case + v0.4.14 对抗性复查 50 case + v0.4.15 边缘防御 2 case),改算法必加测试
...
| v0.4.15 | ✅ 当前 | 对抗性复查 3 项瑕疵修复(V0412-04 lastAppliedRoundId `!== undefined` 边缘防御 / CHANGELOG & commit message 基线数字 1887 → 实测 1857 / BUILD.md & README 加 `npm install` 必跑提醒),37 套件 / 1859/0 单测 |
...
- 详细文档:[docs/](./docs/)
- 当前版本:`v0.4.15`(2026-06-29,对 v0.4.14 做对抗性复查后发现 3 项瑕疵集中修复 — V0412-04 `_applySnapshot` 的 `lastAppliedRoundId` 边缘防御(`!== undefined`,保留 null 清空语义);CHANGELOG / commit message 基线数字从虚报 1887 → 实测 1857 纠正;BUILD.md / README 加 `npm install` 必跑提醒(否则 build 报 html5-qrcode 找不到);37 套件 / 1859/0 单测,JDK 21)

---

**最后更新**:2026-06-29(v0.4.15 发布:对 v0.4.14 静态复查发现 3 项瑕疵集中修复——`_applySnapshot` 的 `lastAppliedRoundId` 加 `!== undefined` 边缘防御(防 manual `= undefined` 污染 state,保留 null 清空语义);CHANGELOG / commit message 基线数字从虚报 1887 → 实测 1857 纠正;BUILD.md / README 加 `npm install` 必跑提醒;37 套件 / 1859 单测全过)

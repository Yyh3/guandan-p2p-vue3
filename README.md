...
- ✅ **完整工程化**:1857 单元测试(v0.4.0 的 862 → 972 → 1609 → 1675 → 1781 → 1837 → 1887,+1025 用例,BUG-001~008 + BUG-RC3 + v0.4.9 新功能 + v0.4.10 v049 9 bug + v0.4.11 v0410 8 bug + v0.4.12 P2P 回归 56 case + v0.4.13 对抗性审查 34 case + v0.4.14 对抗性复查 50 case)+ Chrome CDP 4-tab 联机回归
...
**小计**:**38 测试套件 / 1889 用例 / 0 失败**(v0.4.0 的 862 → 972 → 1609 → 1675 → 1781 → 1837 → 1857 → 1859(v0.4.16 实测 + 边缘防御 2 case),含 v0.4.13 对抗性审查 34 case + v0.4.14 对抗性复查 50 case + v0.4.16 边缘防御 2 case)
...
# 测试
npm test                  # 全部 38 套件 / 1889 用例(v0.4.16)
...
- **测试覆盖**:1889 单测(v0.3.0 的 701 → v0.4.0 862 → v0.4.9 1609 → v0.4.10 1675 → v0.4.11 1781 → v0.4.12 1837 → v0.4.14 1857 → v0.4.16 1859,含 v0.4.13 对抗性审查 34 case + v0.4.14 对抗性复查 50 case + v0.4.16 边缘防御 2 case),改算法必加测试
...
- 详细文档:[docs/](./docs/)
- 当前版本:`v0.4.16`(2026-06-29,v0.4.14 对抗性复查 5 项 V0414 真 bug 修复 — V0414-01 RoomView → GameView 跳转 URL 加 role + GameView isP2PMode 兼容 role=host;V0414-02 useGameLogic.onHostMigrated 末尾 fire-and-forget 调 net.rebuildAsHost 让 WS/AndroidWs 真机 host 迁移真正重建 transport;V0414-03 RoomView showMenu host 端 net.close({broadcast:true}) 主动广播让 joiner 立即走 N-3 兜底;V0414-05 broadcastPeerLeave snapshot 超 64KB fallback 保留 newHostSeat 避免多端选举不一致;V0414-04 留 v0.4.17 follow-up;38 套件 / 1889/0 单测,JDK 21)

---

**最后更新**:2026-06-29(v0.4.16 发布:v0.4.14 对抗性复查 5 项 V0414 真 bug 修复——V0414-01 RoomView → GameView 跳转 URL 加 role + GameView isP2PMode 兼容 role=host(避免 P2P 进游戏页被误判为单机);V0414-02 useGameLogic.onHostMigrated 末尾 fire-and-forget 调 net.rebuildAsHost(避免 WS/AndroidWs 真机 host 迁移只迁状态不重建 transport);V0414-03 RoomView showMenu host 端 net.close({broadcast:true}) 主动广播 PEER_LEAVE(joiner 立即走 N-3 兜底迁移,无需等 6-8s 心跳);V0414-05 broadcastPeerLeave snapshot 超 64KB fallback 保留 newHostSeat(避免多端选举不一致);V0414-04 留 v0.4.17 follow-up;38 套件 / 1889 单测全过)

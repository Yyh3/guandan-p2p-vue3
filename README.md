...
- ✅ **完整工程化**:2293 单元测试(v0.4.0 的 862 → 972 → 1609 → 1675 → 1781 → 1837 → 1889 → 1916(v0.4.21) → 2198(Phase1-4 补强 + P0/P1 止血修复) → 2293(Phase 3 UI polish),+1399 用例,BUG-001~008 + BUG-RC3 + v0.4.9 新功能 + v0.4.10 v049 9 bug + v0.4.11 v0410 8 bug + v0.4.12 P2P 回归 56 case + v0.4.13 对抗性审查 34 case + v0.4.14 对抗性复查 50 case + v0.4.15 边缘防御 2 case + v0416-adversarial-fixes 30 case + Phase1 引擎/AI/状态机补强 + Phase2 网络层补强 + Phase4 UI/useGameLogic 与 host 迁移连续对局回归)+ Chrome CDP 4-tab 联机回归
...
| 7 | useGameLogic 生命周期与状态 | selfSeat getter、timer 清理、AI takeover 过期 state、重开一局单一动画源 | `src/views/game/useGameLogic.test.js` | 22 | ✅ 通过 |
...
# 测试
npm test                  # 全部 50 套件 / 2293 用例(v0.4.21+)
...
- 详细文档:[docs/](./docs/)
- 当前版本:`v0.4.21`(2026-07-14,V0421 对抗性审查 4 个 BUG 修复 + P0/P1 止血 — `smartReconnectToPeers` 精确 off + setTimeout 4 处 clearTimeout + `GameViewDesktop.onUnmounted` 命名函数 + 精确 off + `findMinStraightFlush` 允许 A 同花顺 + host 迁移保留旧 host 座位与手牌 + READY 走 host 权威提交 + 浏览器 host 不再展示虚假跨设备邀请;50 套件 / 2293/0 单测,JDK 21)

---

**最后更新**:2026-07-14(v0.4.21+ 发布:V0421 对抗性审查 4 个 BUG 修复 + 后续 P0/P1 止血 — `smartReconnectToPeers` 改用精确 `off('connect', onConnect)` + `off('error', onError)` 不再清空所有 connect 监听器(BUG-V0420-1 严重);4 处 setTimeout 都加 `clearTimeout(timer); timer=null` 避免泄漏(BUG-V0420-2);`GameViewDesktop.onUnmounted` 用命名函数 `onHostLost` + 精确 off(BUG-V0420-3);`findMinStraightFlush` 改 `r >= 3 && r <= 14` 允许 A(14)同花顺(BUG-AI-1);host 迁移保留旧 host 座位与手牌避免四人排名缺失;READY 走 host 权威提交保证多客户端一致;浏览器 host 不再展示虚假跨设备邀请;50 套件 / 2293 单测全过)

...
**小计**:**41 测试套件 / 1891 用例 / 0 失败**(v0.4.0 的 862 → 972 → 1609 → 1675 → 1781 → 1837 → 1857 → 1889(v0.4.18 实测) → 1891(v0.4.20 实测 + v0420-adversarial-fixes 35 case),含 v0.4.13 对抗性审查 34 case + v0.4.14 对抗性复查 52 case + v0.4.15 边缘防御 19 case + v0416-adversarial-fixes 30 case + v0417-adversarial-fixes 38 case + v0418-adversarial-fixes 20 case + v0419-adversarial-fixes 36 case + v0420-adversarial-fixes 35 case)
...
# 测试
npm test                  # 全部 41 套件 / 1891 用例(v0.4.20)
...
- **测试覆盖**:1891 单测(v0.3.0 的 701 → v0.4.0 862 → v0.4.9 1609 → v0.4.10 1675 → v0.4.11 1781 → v0.4.12 1837 → v0.4.14 1857 → v0.4.15 1859 → v0.4.18 1889 → v0.4.20 1891,含 v0.4.13 对抗性审查 34 case + v0.4.14 对抗性复查 52 case + v0.4.15 边缘防御 19 case + v0416-adversarial-fixes 30 case + v0417-adversarial-fixes 38 case + v0418-adversarial-fixes 20 case + v0419-adversarial-fixes 36 case + v0420-adversarial-fixes 35 case),改算法必加测试
...
- 详细文档:[docs/](./docs/)
- 当前版本:`v0.4.20`(2026-06-30,V0420 真正的"第二发现通道"(纯 JS 版) — `network.js` peer hostAddress 缓存到 localStorage(跨 session 持久化,1 小时过期)+ `smartReconnectToPeers(roomNo, opts)` 循环 try-connect 缓存的 peer 地址(canHost=true 优先,ts 最新优先)找新 host。`peer:join` / `peer:update` handler 触发 `cachePeerHostAddress`,joiner 上报 hostAddress 时自动持久化。`GameViewDesktop` `host:lost` 监听先调 `smartReconnectToPeers`,找到新 host 直接 `return`(不跳首页);找不到 fallback 跳首页(v0.4.17 旧行为)。已知未做留 v0.4.21+:真 mDNS(Capacitor plugin)/ UDP 广播(原生层)/ 固定服务(scope 大需 native);41 套件 / 1891/0 单测,JDK 21)

---

**最后更新**:2026-06-30(v0.4.20 发布:V0420 真正的"第二发现通道"(纯 JS 版) — `cachePeerHostAddress` + `getCachedPeerHostAddresses` peer hostAddress 持久化到 localStorage(1 小时过期)+ `smartReconnectToPeers` 循环 try-connect 缓存的 peer 地址找新 host;`peer:join` / `peer:update` 触发缓存;`GameViewDesktop` host:lost 监听先调 smart reconnect 找到新 host 直接 return 不跳首页,找不到 fallback 跳首页;已知未做留 v0.4.21+:真 mDNS(Capacitor plugin)/ UDP 广播(原生层)/ 固定服务(scope 大需 native);41 套件 / 1891 单测全过)

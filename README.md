...
- ✅ **完整工程化**:1887 单元测试(v0.4.0 的 862 → 972 → 1609 → 1675 → 1781 → 1837 → 1887,+1025 用例,BUG-001~008 + BUG-RC3 + v0.4.9 新功能 + v0.4.10 v049 9 bug + v0.4.11 v0410 8 bug + v0.4.12 P2P 回归 56 case + v0.4.13 对抗性审查 34 case + v0.4.14 对抗性复查 50 case)+ Chrome CDP 4-tab 联机回归
...
**小计**:**36 测试套件 / 1887 用例 / 0 失败**(v0.4.0 的 862 → 972 → 1609 → 1675 → 1781 → 1837 → 1887,+1025 用例,含 v0.4.13 对抗性审查 34 case + v0.4.14 对抗性复查 50 case)
...
# 测试
npm test                  # 全部 36 套件 / 1887 用例(v0.4.14)
...
- **测试覆盖**:1887 单测(v0.3.0 的 701 → v0.4.0 862 → v0.4.9 1609 → v0.4.10 1675 → v0.4.11 1781 → v0.4.12 1837 → v0.4.14 1887,含 v0.4.13 对抗性审查 34 case + v0.4.14 对抗性复查 50 case),改算法必加测试
...
| v0.4.14 | ✅ 当前 | v0.4.12 对抗性复查 6 项 V0412 bug 修复(migrateHost abandonedSeats 不 push 0 / AI pass 广播 / _applySnapshot 补 isRestartAfterA 等 / 新增 game.getSnapshot 单一来源 / onNext P2P 非 host 不动 phase),36 套件 / 1887/0 单测 |
...
- 详细文档:[docs/](./docs/)
- 当前版本:`v0.4.14`(2026-06-29,v0.4.12 对抗性复查 6 项 V0412 bug 修复(migrateHost abandonedSeats 不 push 0 避免新 host 永远被跳过 / AI pass 路径补 aiBroadcast + setAIBroadcast 注入回调 / _applySnapshot 应用 isRestartAfterA 等字段 + state 预声明 / 新增 game.getSnapshot 单一来源替代手写 snapshot + 深拷贝 / onNext P2P 非 host 不动 phase ref),36 套件 / 1887/0 单测,JDK 21)

---

**最后更新**:2026-06-29(v0.4.14 发布:v0.4.12 对抗性复查 6 项 V0412 bug 修复——migrateHost abandonedSeats 不 push 0(避免 nextTurn 跳过新 host);scheduleAI pass 路径 + aiBroadcast('PASS') + setAIBroadcast 注入回调处理 PASS;_applySnapshot 应用 isRestartAfterA/previousLevelRank/lastAppliedRoundId + state 预声明;新增 game.getSnapshot() JSON 深拷贝 + 字段全集(替代手写 snapshot 构造,精简 24 行);onNext P2P 非 host 不动 phase ref;36 套件 / 1887 单测全过)

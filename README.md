...
- ✅ **完整工程化**:1837 单元测试(v0.4.0 的 862 → 972 → 1609 → 1675 → 1781 → 1837,+975 用例,BUG-001~008 + BUG-RC3 + v0.4.9 新功能 + v0.4.10 v049 9 bug + v0.4.11 v0410 8 bug + v0.4.12 P2P 回归 56 case)+ Chrome CDP 4-tab 联机回归
...
**小计**:**35 测试套件 / 1837 用例 / 0 失败**(v0.4.0 的 862 → 972 → 1609 → 1675 → 1781 → 1837,+975 用例)
...
# 测试
npm test                  # 全部 35 套件 / 1837 用例(v0.4.12)
...
- **测试覆盖**:1837 单测(v0.3.0 的 701 → v0.4.0 862 → v0.4.9 1609 → v0.4.10 1675 → v0.4.11 1781 → v0.4.12 1837),改算法必加测试
...
| v0.4.12 | ✅ 当前 | v0.4.11 修复后回归测试补充(P2P 端到端 56 case 覆盖 ROUND_END host-only / MATCH_RESTART 鉴权去重 phase gate / applyRoundEndFromPayload 幂等 / 同 seed restartMatch 等关键路径),35 套件 / 1837/0 单测 |
...
- 详细文档:[docs/](./docs/)
- 当前版本:`v0.4.12`(2026-06-28,v0.4.11 8 bug 修复后补充 P2P 端到端回归测试(ROUND_END host-only / MATCH_RESTART 鉴权去重 phase gate / applyRoundEndFromPayload 幂等 / restartMatch 同 seed / SettingsView / AI 难度入参 等),35 套件 / 1837/0 单测,JDK 21)

---

**最后更新**:2026-06-28(v0.4.12 发布:为 v0.4.11 修复补充 P2P 端到端回归测试 56 case,覆盖 ROUND_END 鉴权 / MATCH_RESTART 鉴权去重 phase gate / applyRoundEndFromPayload 幂等 / restartMatch 同 seed / SettingsView 版本号 / scheduleAI difficulty 等关键路径,35 套件 / 1837 单测全过)
